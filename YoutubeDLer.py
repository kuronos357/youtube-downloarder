...
class YouTubeDownloader:
    ...
    def _get_format_options(self, format_choice: str) -> Tuple[str, List[Dict]]:
        ...
        if not format_config:
            ...
            if format_choice.lower() == 'mp3':
                # 音声専用フォーマットを優先
                primary_format = primary_format.replace('best[', 'bestaudio[').replace('best/', 'bestaudio/')
                fallback_format = fallback_format.replace('best', 'bestaudio')
            ...
            # MP3形式で明示的にポストプロセッサが設定されていない場合は自動追加
        ...
        logger.info(f"使用フォーマット: {format_str}")
    ...
    def _create_download_options(self, output_dir: str, format_choice: str) -> Dict:
        ...
        # デバッグ：最終的なオプションをログ出力
        logger.debug(f"ダウンロードオプション: {json.dumps(options, indent=2, ensure_ascii=False)}")
    ...
    def _download_with_multiple_methods(self, url: str, output_dir: str, format_choice: str) -> bool:
        ...
        if self.config.get('enable_android_fallback', True):
            methods.append(("Android クライアント", lambda: self._download_android_client(url, output_dir, format_choice)))
        
        methods.append(("最低品質フォールバック", lambda: self._download_minimal(url, output_dir, format_choice)))
    ...
    def _download_android_client(self, url: str, output_dir: str, format_choice: str):
        ...
        if format_choice.lower() == 'mp3':
            ...
            options['format'] = f'best[height<={fallback_height}]/best'
    ...
    def _download_minimal(self, url: str, output_dir: str, format_choice: str):
        """最小限の設定でダウンロード"""
        options = {
            'outtmpl': str(Path(output_dir) / '%(title)s.%(ext)s'),
            'no_warnings': True,
        ...
        # フォーマットに応じて最小限の設定
        if format_choice.lower() == 'mp3':
            options['format'] = 'bestaudio/worst'
            options['postprocessors'] = [{
                'key': 'FFmpegExtractAudio',
                'preferredcodec': 'mp3',
                'preferredquality': '128'  # 最小品質
            }]
        else:
    ...
    def validate_config(self) -> bool:
        ...
        # 設定内容をデバッグ出力
        logger.info(f"読み込まれた設定: {json.dumps(self.config, indent=2, ensure_ascii=False)}")
    ...
    def print_available_formats(self):
        ...
        if format_options:
            ...
            logger.warning("format_options が設定ファイルで定義されていません")
    ...
    def run(self):
        ...
        logger.info(f"ログレベルを {log_level} に設定")
        ...
        logger.info(f"使用する保存先： {output_dir}")
        logger.info(f"使用するフォーマット： {format_choice}")
